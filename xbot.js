...
async function starts() {
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			const BotWea2 = ["083144780782@s.whatsapp.net"] // ubah aja gapapa
			...
			mess = {
				...
				only: {
					...
					userB: `*Hallo Kak* ${pushname2} *Kayanya Kamu Blm Terdaftar Sebagai Teman X BOT*\n*Ketik ${prefix}daftar nama|umur*\n*Contoh ${prefix}daftar Zaki|17*`,
		    ...
		    case prefix+'map':
				...
				case prefix+'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *X BOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
